    ...
    if (isset($_POST['bulk_action'])) {
        ...
        try {
            if ($action === 'delete') {
                foreach ($ids as $id) {
                    $id = (int)$id;
                    $stmt = $pdo->prepare("SELECT COUNT(*) FROM classes WHERE level_id = ?");
                    $stmt->execute([$id]);
                    if ($stmt->fetchColumn() > 0) {
                        throw new Exception('Cannot delete levels with classes.');
                    }
                }
            }
            ...
            if ($action === 'delete') {
                $stmt = $pdo->prepare("DELETE FROM levels WHERE id IN ($placeholders)");
            } elseif ($action === 'activate') {
                $stmt = $pdo->prepare("UPDATE levels SET status = 'active' WHERE id IN ($placeholders)");
            } elseif ($action === 'deactivate') {
                $stmt = $pdo->prepare("UPDATE levels SET status = 'inactive' WHERE id IN ($placeholders)");
            } else {
    ...
    if (isset($_POST['action'])) {
        ...
        try {
            if ($action === 'update') {
                ...
                if (empty($name)) throw new Exception('Level name is required.');
                if (!in_array($status, ['active', 'inactive'])) throw new Exception('Invalid status.');

                $stmt = $pdo->prepare("SELECT id FROM levels WHERE name = ? AND id != ?");
                $stmt->execute([$name, $id]);
                if ($stmt->fetch()) throw new Exception('Level name already exists.');

                $stmt = $pdo->prepare("UPDATE levels SET name = ?, status = ? WHERE id = ?");
                ...
                $stmt = $pdo->prepare("UPDATE levels SET status = CASE WHEN status = 'active' THEN 'inactive' ELSE 'active' END WHERE id = ?");
                $stmt->execute([$id]);
                $new = $pdo->query("SELECT status FROM levels WHERE id = $id")->fetchColumn();
                echo json_encode(['success' => true, 'new_status' => $new, 'message' => 'Status updated successfully!']);
            } elseif ($action === 'delete') {
                $stmt = $pdo->prepare("SELECT COUNT(*) FROM classes WHERE level_id = ?");
                $stmt->execute([$id]);
                if ($stmt->fetchColumn() > 0) throw new Exception('Cannot delete: Level has classes.');
...
$stmt = $pdo->prepare("SELECT COUNT(*) FROM levels WHERE $where");
$stmt->execute($params);
$total = (int)$stmt->fetchColumn();
$pages = max(1, ceil($total / $limit));

$stmt = $pdo->prepare("SELECT * FROM levels WHERE $where ORDER BY name LIMIT ? OFFSET ?");
$params[] = $limit;
$params[] = $offset;
$stmt->execute($params);
...
<body>
                    ...
                    <tbody>
                        <?php foreach ($items as $i => $item): ?>
                            <tr id="level-<?= $item['id'] ?>">
                                ...
                                <td><?= (($page - 1) * 10) + $i + 1 ?></td>
                                <td class="level-name"><?= htmlspecialchars($item['name']) ?></td>
                                <td>
    ...
    <div class="modal-overlay" id="editModal">
        <div class="modal">
            <div class="modal-header">
                <h3><i class="fas fa-edit"></i> Edit Level</h3>
            ...
            <form id="editForm">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="editLevelName"><i class="fas fa-tag"></i> Level Name *</label>
                        <input type="text" id="editLevelName" name="name" class="form-control" required>
                    </div>
                    <div class="form-group">
    ...
    <script>
        ...
        function openEditModal(id, name, status) {
            ...
            document.getElementById('editLevelName').focus();
        }
            ...
            fetch('manage_levels.php', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        const row = document.getElementById(`level-${currentEditId}`);
                        if (row) {
                            row.querySelector('.level-name').textContent = data.new_name;
                        ...
                        showNotification(data.message || 'Failed to update level!', 'error');
                    }
                })
                .catch(error => {
        ...
        function applyBulkAction() {
            ...
            if (ids.length === 0) {
                showNotification('Please select at least one level.', 'warning');
                return;
            }
            ...
            let icon = '';
            if (action === 'delete') {
                confirmMessage = `Are you sure you want to delete ${ids.length} selected level(s)?`;
                icon = '🗑️';
            } else if (action === 'activate') {
                confirmMessage = `Are you sure you want to activate ${ids.length} selected level(s)?`;
                icon = '✅';
            } else if (action === 'deactivate') {
                confirmMessage = `Are you sure you want to deactivate ${ids.length} selected level(s)?`;
                icon = '⛔';
            }
            ...
            fetch('manage_levels.php', {
                ...
                .then(data => {
                    if (data.success) {
                        let message = '';
                        if (action === 'delete') message = `${ids.length} level(s) deleted successfully!`;
                        else if (action === 'activate') message = `${ids.length} level(s) activated successfully!`;
                        else if (action === 'deactivate') message = `${ids.length} level(s) deactivated successfully!`;
        ...
        function toggleStatus(id) {
            ...
            if (!confirm(`${icon} Are you sure you want to ${action} this level?`)) return;
        ...
        function deleteRecord(id) {
            const row = document.getElementById(`level-${id}`);
            const levelName = row.querySelector('.level-name').textContent;

            if (!confirm(`🗑️ Are you sure you want to delete level "${levelName}"?`)) return;
