...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$level_id || empty($name)) {
        $error = "Please select a level and enter a class name.";
    } elseif (!in_array($level_id, array_column($levels, 'id'))) {
        $error = "Invalid level selected.";
    } else {
        try {
            // Check duplicate within same level
            $stmt = $pdo->prepare("SELECT id FROM classes WHERE level_id = ? AND name = ?");
            $stmt->execute([$level_id, $name]);
            if ($stmt->fetch()) {
                $error = "This class already exists in the selected level.";
            } else {
                $stmt = $pdo->prepare("INSERT INTO classes (level_id, name) VALUES (?, ?)");
                if ($stmt->execute([$level_id, $name])) {
...
<body>
    ...
    <script>
        ...
        // Auto-hide success message & reset form
