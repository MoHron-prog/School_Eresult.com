...
// Fetch levels for first dropdown
$levels = [];
$classes = [];
$selected_level = null;

try {
    $stmt = $pdo->query("SELECT id, name FROM levels ORDER BY name");
    $levels = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    $error = "Failed to load levels.";
}

// If level is selected via POST or AJAX (not implemented here), load classes
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['level_id'])) {
    $selected_level = (int)$_POST['level_id'];
    try {
        $stmt = $pdo->prepare("SELECT id, name FROM classes WHERE level_id = ? ORDER BY name");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$level_id || !$class_id || empty($name)) {
        $error = "Please select level, class, and enter a stream name.";
    } else {
        // Validate class belongs to level
        try {
            $stmt = $pdo->prepare("SELECT id FROM classes WHERE id = ? AND level_id = ?");
            $stmt->execute([$class_id, $level_id]);
            if (!$stmt->fetch()) {
                $error = "Invalid class selection.";
            } else {
                // Check duplicate stream in same class
                $stmt = $pdo->prepare("SELECT id FROM streams WHERE class_id = ? AND name = ?");
                $stmt->execute([$class_id, $name]);
                if ($stmt->fetch()) {
                    $error = "This stream already exists in the selected class.";
                } else {
                    $stmt = $pdo->prepare("INSERT INTO streams (class_id, name) VALUES (?, ?)");
                    if ($stmt->execute([$class_id, $name])) {
    ...
    // Re-fetch classes for form repopulation
    if ($level_id) {
        try {
            $stmt = $pdo->prepare("SELECT id, name FROM classes WHERE level_id = ? ORDER BY name");
...
<body>
            ...
            <div class="form-container">
                ...
                <form id="streamForm" method="POST">
                    <div class="form-group">
                        ...
                        <select id="level_id" name="level_id" class="form-control" required onchange="this.form.submit()">
                    ...
                    <div class="form-group">
                        ...
                        <input
                            ...
                            name="name"
                            class="form-control"
                            placeholder="e.g., North, South, Morning, Evening"
    ...
    <script>
        ...
        // Auto-hide success message & reset
